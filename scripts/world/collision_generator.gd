...
static func build_collision_shape(chunk_data: ChunkData) -> ConcavePolygonShape3D:
	...
	for lz: int in range(resolution - 1):
		for lx: int in range(resolution - 1):
			...
			var h10: float = chunk_data.heightmap[lz * resolution + lx + 1]
			var h01: float = chunk_data.heightmap[(lz + 1) * resolution + lx]
			var h11: float = chunk_data.heightmap[(lz + 1) * resolution + lx + 1]
